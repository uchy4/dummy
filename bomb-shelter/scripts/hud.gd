...
func setup(colors: Array[Color], touch := false) -> void:
	...
	# Win banner: a slim strip at the top, so the in-world podium ceremony in
	# the finish hall stays visible. The nearly-clear full-rect overlay still
	# catches the tap-anywhere-for-rematch input.
...
## End-of-match banner. The celebration itself happens in the world — the
## camera cuts to the finish hall where Ceremony animates the top three on
## the real podium — so the HUD only names the winner and the rematch key.
func show_win_banner(entries: Array[Dictionary], time: float, reason: String) -> void:
	if entries.is_empty():
